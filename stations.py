...
def get_json(feature, _id):
    if feature not in ["liveboard", "vehicle"]:
        raise ValueError(
                """
                The feature should be 'liveboard' or 'vehicle'
                """)
    ...
    try:
        ...
        click.echo(
            ...
            There was no JSON returned
            by the iRail API!""")
        click.echo("The system looked for " + feature + " " + _id)
        raise SystemExit(0)
...
def station_vehicles(station_id, vehicles = []):  # good reason for this!
    ...
    try:
        ...
        click.echo(
            ...
            ran this script at night,
            and there are no trains leaving
            soon. You should run this during
            the day in order to get 
            relevant results.
            """)
        raise SystemExit(0)
    return vehicles
...
def vehicle_stops(vehicle_id,
                  station_name,
                  max_transit_time=1800):
    """
    Get all stops for one vehicle
...
def station_stops(station_id,
                  station_name,
                  max_transit_time=1800,
                  vehicles=None):
    """
    Get all stops that are
    within a certain timerange
    from a station
    """
        ...
        for vehicle in vs:
            ...
            for station, data in stops.items():
                if station not in destinations:
                    ...
                    destinations[station]["transit_time"] = avg(
                            destinations[station]["transit_time"],
                            data["transit_time"])
    ...
    with click.progressbar(destinations.items()) as ds:
        for destination, data in ds:
            ...
            if include_locations:
                destination_location = get_lat_lng(
...
def show_station_stops(stations):
    click.secho("Station".ljust(20) +
                "Avg. transit time".ljust(20) +
                "Overall frequency",
                reverse = True)
    for (station,
         score,
         ...
         overall_frequency,
         morning_frequency,
         evening_frequency) in stations:
        click.echo(station.ljust(25) +
                   str(avg_transit_time).ljust(20) +
                   str(overall_frequency))


def get_station(suggestion):
    headers = {'Content-type': 'application/json',
               'Accept': 'text/plain'}
    json_data = requests.get("https://irail.be/stations/NMBS/",
                             {"q" : suggestion},
    ...
    if len(suggestions) == 1:
        ...
        station_index = click.prompt("""
            ...
            """.format(suggestion), type=int)

    return ("BE.NMBS." +
            suggestions[station_index]["@id"][30:],
            suggestions[station_index]["name"])
...
def get_distance(station_one_id, station_two_id):
    ...
    return great_circle(station_one_location,
                        station_two_location).meters
...
@click.argument("station_suggestion",
                required=True,
                type=str)
@click.argument("max_transit_time_in_minutes",
                required=False,
                default=30,
                type=int)
@click.option("--day",
              ...
              default=False,
              required=False)
@click.option("--include_locations",
              "-l",
              is_flag=True,
              default=False,
              required=False)
def main(station_suggestion,
         max_transit_time_in_minutes,
         day,
         include_locations):
    ...
    if day:
        ...
        with click.progressbar(range(24)) as hours:
            for _ in hours:
                vehicles = station_vehicles(station_id)
                sleep(3600)
    stops = station_stops(station_id,
                          station_name,
                          max_transit_time_in_minutes * 60,
                          vehicles)
    if include_locations:
        station_location = get_lat_lng(station_id)
    processed_stops = process_destinations(stops,
                                           include_locations,
                                           station_location)
